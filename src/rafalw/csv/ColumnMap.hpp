...
public:
    ...
    {
    public:
        ColumnNotFoundError(const std::string_view& file, const std::string_view& name) :
    ...
    auto get(const std::string_view& name) const -> const Column&
    {
        try {
...
private:
    ...
    std::map<std::string_view, Column*> m_map;
};
