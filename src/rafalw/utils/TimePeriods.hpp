#ifndef RAFALW_UTILS_TIMEPERIODS_HPP_
#define RAFALW_UTILS_TIMEPERIODS_HPP_
...
public:
    ...
    {
        rafalw_utils_assert(begin_time >= timeMin());
        rafalw_utils_assert(begin_time <= timeMax());
        ...
        rafalw_utils_assert(!p.empty());
        rafalw_utils_assert(!(begin_time < p.back().beginTime));
    ...
    {
        rafalw_utils_assert(t >= timeMin());
        rafalw_utils_assert(t <= timeMax());

        auto it = std::lower_bound(data().begin(), data().end(), t, [](const Period& p, Time t) {
            return p.endTime <= t;
        });

        rafalw_utils_assert(it != data().end());
        rafalw_utils_assert(t >= it->beginTime && t < it->endTime);
...
namespace detail {
    ...
    inline auto time_periods_process(const TimePeriods& tps1, const TimePeriods& tps2, F&& f) -> void
    {
        ...
        while (true)
        {
            rafalw_utils_assert(it1 != tps1.end());
            rafalw_utils_assert(it2 != tps2.end());
...
#endif // RAFALW_UTILS_TIMEPERIODS_HPP_
